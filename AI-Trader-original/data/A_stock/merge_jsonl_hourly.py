"""
Convert A-share hourly CSV data to SQLite database.

This script reads A-share hourly price CSV files and saves them to the prices database.

Usage:
    python data/A_stock/merge_jsonl_hourly.py
"""
...
def convert_hourly_to_db(
    csv_path: str = "A_stock_data/A_stock_hourly.csv",
    stock_name_csv: str = "A_stock_data/sse_50_weight.csv",
) -> None:
    """Convert A-share hourly CSV data to database.

    Args:
        csv_path: Path to the A-share hourly price CSV file
    ...
    stock_name_csv_path = Path(stock_name_csv)
    if stock_name_csv_path.exists():
        print(f"Reading stock names from: {stock_name_csv_path}")
        name_df = pd.read_csv(stock_name_csv_path)
        stock_name_map = dict(zip(name_df["con_code"], name_df["stock_name"]))
        print(f"Loaded {len(stock_name_map)} stock names")
    else:
        print(f"Warning: Stock name file not found: {stock_name_csv_path}")
        stock_name_map = {}
    ...
    for stock_code, group_df in grouped:
        ...
        for idx, row in group_df.iterrows():
            datetime_str = str(row["trade_date"])
            ...
            try:
                ...
                print(f"  Error adding {stock_code} at {datetime_formatted}: {e}")
...
if __name__ == "__main__":
    print("=" * 60)
    print("A-Share Hourly Data Converter to Database")
    print("=" * 60)

    current_dir = Path(__file__).parent
    csv_path = current_dir / "A_stock_data" / "A_stock_hourly.csv"
    stock_name_csv = current_dir / "A_stock_data" / "sse_50_weight.csv"

    convert_hourly_to_db(str(csv_path), str(stock_name_csv))
